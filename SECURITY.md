...
partition. Once completed, it contains a live secret. Piccie imports it into
protected `/data/local.json` and deletes the boot copy before starting the
engine. Because deleted FAT data may be forensically recoverable, keep the card
inside the booth and use an Object Read & Write token restricted to only the
Piccie bucket. Rotate the token if the card is lost or reused.
